...
def test_index_signup(client):
    """サインアップを実行する"""
    rv = signup(client, "das", "das@321.com", "1")
    assert "das" in rv.data.decode()
    rv = client.get("/")
...
def test_upload_signup_get(client):
    signup(client, "das", "das@321.com", "1")
...
def test_upload_signup_post(client):
    signup(client, "das", "das@321.com", "1")
    rv = upload_image(client, "detector/testdata/unnamed.jpeg")
...
def test_detect_no_user_image(client):
    signup(client, "das", "das@321.com", "1")
    upload_image(client, "detector/testdata/unnamed.jpeg")
    # 存在しないIDを指定する
